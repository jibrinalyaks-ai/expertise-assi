...
// Serve static files - CRITICAL: Must come before routes
...
app.get('/api/dashboard/stats', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    const [students, teachers, classes, pendingAdmissions, announcements, reports] = await Promise.all([
      prisma.student.count(),
      prisma.teacher.count(),
      prisma.class.count(),
      prisma.admission.count({ where: { status: 'pending' } }),
      prisma.announcement.count(),
      prisma.missingReport.count({ where: { status: 'pending' } })
    ]);

    res.json({ students, teachers, classes, pendingAdmissions, announcements, reports });
  } catch (error) {
...
// ==================== SERVE FRONTEND ====================
// This MUST be the last route - catch-all for SPA
app.get('*', (req, res) => {
...
async function start() {
  try {
    ...
    try {
      ...
      console.warn('⚠️ Database sync note:', e.message);
    }
