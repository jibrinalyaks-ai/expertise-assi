...
async function seedData() {
  try {
    const adminExists = await prisma.user.findUnique({ where: { email: 'admin@assi.edu.ng' } });
    if (!adminExists) {
      const hashedPassword = await bcrypt.hash('admin@access010', 10);
      await prisma.user.create({
        data: { email: 'admin@assi.edu.ng', password: hashedPassword, name: 'Super Admin', role: 'ADMIN' }
      });
      console.log('✅ Default admin created');
    }

    const teacherExists = await prisma.user.findUnique({ where: { email: 'teacher@assi.edu.ng' } });
    if (!teacherExists) {
      const hashedPassword = await bcrypt.hash('teacher123', 10);
      const teacherUser = await prisma.user.create({
        data: { email: 'teacher@assi.edu.ng', password: hashedPassword, name: 'Mrs. Adeyemi', role: 'TEACHER' }
      });
      await prisma.teacher.create({
        data: { userId: teacherUser.id, subject: 'Mathematics', classAssigned: 'Primary 3' }
      });
      console.log('✅ Demo teacher created');
    }

    const studentExists = await prisma.user.findUnique({ where: { email: 'student@assi.edu.ng' } });
    if (!studentExists) {
      const hashedPassword = await bcrypt.hash('password', 10);
      const studentUser = await prisma.user.create({
        data: { email: 'student@assi.edu.ng', password: hashedPassword, name: 'Amara Okonkwo', role: 'STUDENT' }
      });
      await prisma.student.create({
        data: { userId: studentUser.id, rollNo: 'ASSI/2026/001', class: 'Primary 3', parentPhone: '08012345678' }
      });
    ...
    if (classCount === 0) {
      const classes = ['Nursery 1', 'Nursery 2', 'Nursery 3', 'Primary 1', 'Primary 2', 'Primary 3', 'Primary 4', 'Primary 5', 'Primary 6'];
